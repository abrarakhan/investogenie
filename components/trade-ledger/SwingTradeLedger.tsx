...
function TradeCard({ trade, today }: { trade: SwingLedgerTrade; today: string }) {
  ...
  return (
    <article className="rounded-lg border border-white/10 bg-white/[0.025] p-5">
      ...
      {trade.exits.length > 0 && <details className="mt-3"><summary className="cursor-pointer text-sm font-semibold text-white/45 hover:text-white">Sale history · {trade.exits.length}</summary><div className="mt-2 divide-y divide-white/8 rounded-lg border border-white/8 bg-black/20 px-3">{trade.exits.map((exit) => <div key={exit.id} className="py-3 text-xs"><div className="grid gap-1 sm:grid-cols-5 sm:items-center"><span>{exit.soldOn}</span><span>{exit.quantity.toLocaleString("en-IN")} shares</span><span>@ {price(exit.exitPrice)}</span><span className={exit.realizedPnlValue >= 0 ? "text-emerald-300" : "text-rose-300"}>{money(exit.realizedPnlValue, trade.currency)}</span><span className="text-white/40">{exit.reason ?? "Sale"}</span><details className="sm:col-span-5"><summary className="min-h-9 cursor-pointer list-none rounded-lg px-2 py-2 font-semibold text-[var(--ig-accent)] hover:bg-white/5 [&::-webkit-details-marker]:hidden">Edit sale</summary><form action={updateSwingTradeSale} className="mt-2 grid gap-3 rounded-lg border border-white/10 bg-black/30 p-3 sm:grid-cols-2 lg:grid-cols-5"><input type="hidden" name="tradeId" value={trade.id} /><input type="hidden" name="saleId" value={exit.id} /><input type="hidden" name="market" value={trade.market} /><Field label="Sale date"><input name="soldOn" type="date" required min={trade.boughtOn} max={today} defaultValue={exit.soldOn} className="field" /></Field><Field label={`Quantity (max ${(trade.remainingQuantity + exit.quantity).toLocaleString("en-IN")})`}><input name="soldQuantity" type="number" min="0.000001" max={trade.remainingQuantity + exit.quantity} step="any" required defaultValue={exit.quantity} className="field" /></Field><Field label="Sale price"><input name="exitPrice" type="number" min="0.000001" step="any" required defaultValue={exit.exitPrice} className="field" /></Field><Field label="Reason"><select name="saleReason" defaultValue={exit.reason ?? "Manual exit"} className="field bg-[#090c12]"><option>Partial profit booking</option><option>Target reached</option><option>Trailing stop</option><option>Stop loss</option><option>Holding window expired</option><option>Manual exit</option></select></Field><button className="h-11 self-end rounded-lg border border-[var(--ig-accent)]/40 bg-[var(--ig-accent)]/10 px-4 font-semibold text-[var(--ig-accent)] hover:bg-[var(--ig-accent)]/15">Save sale</button></form></details></div></div>)}</div></details>}
